...
public class Main extends Application {
          ...
          rect.setOnMouseReleased(
                  event -> {
                       if (logic.move(this.findBy(momento.getX(), momento.getY()), this.findBy(event.getX(), event.getY()))) {
                            rect.setX(((int) event.getX() / 60) * 60 + 5);
                            rect.setY(((int) event.getY() / 60) * 60 + 5);
                       } else {
                            rect.setX(((int) momento.getX() / 60) * 60 + 5);
                            rect.setY(((int) momento.getY() / 60) * 60 + 5);
                       }
                  }
          );
          return rect;
          ...
          for (int y = 0; y != this.size; y++) {
               for (int x = 0; x != this.size; x++) {
                    panel.getChildren().add(
                            this.buildRectangle(x, y, 60, (x + y) % 2 == 0)
                    );
               }
          }
          return panel;
     ...
     public void add(Figure figure, Group grid) {
          ...
          grid.getChildren().add(
                  this.buildFigure(position.x * 60 + 5, position.y * 60 + 5, 30, figure.icon()
                  )
          );
     }
